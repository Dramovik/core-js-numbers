...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthX = Math.abs(Math.sqrt(x1 ** 2 + y1 ** 2));
  const lengthY = Math.abs(Math.sqrt(x2 ** 2 + y2 ** 2));
  const angle = Math.acos(scalar / (lengthX * lengthY));
  return angle;
}
